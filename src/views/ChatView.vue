...
async function handleClearChat() {
  const ok = await confirmDialog.confirm('确定要清空当前对话吗?', {
    title: '清空对话'
  })
  if (!ok) return
  messages.value = []
...
async function handleDeleteConversation(id) {
  const ok = await confirmDialog.confirm('确定要删除这个对话吗?', {
    title: '删除对话'
  })
  if (!ok) return
...
function toggleTheme() {
  theme.value =
    theme.value === 'light' ? 'dark' : theme.value === 'dark' ? '' : 'light'
      ...
      <div ref="messageContainer" class="messages-container u-scrollbar">
        <div v-if="messages.length === 0" class="empty-state">
          ...
          <div class="quick-actions">
            <button
              class="quick-btn"
              @click="handleQuickPrompt('帮我写一段Python代码')"
            >
              <svg
                width="16"
                height="16"
            ...
            <button
              class="quick-btn"
              @click="handleQuickPrompt('解释这段代码的功能')"
            >
              <svg
                width="16"
                height="16"
            ...
            <button
              class="quick-btn"
              @click="handleQuickPrompt('生成数据可视化图表')"
            >
              <svg
                width="16"
                height="16"
        ...
        <TransitionGroup
          tag="div"
          :name="enableMessageTransition ? 'list' : ''"
          class="message-list"
        >
          <div
